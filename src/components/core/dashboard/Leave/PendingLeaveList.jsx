            ...
            <tbody>
              {pendingLeaves.map((leave, index) => (
                <tr key={index} className=''>
                  ...
                  <td className="py-2 px-4 border-b border-slate-200 text-yellow-600 ">Pending</td>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <div className=' flex items-center justify-center'><p className='text-xl font-semibold text-zinc-700'>No pending leave requests.....</p></div>
        )}
      </div>
    </div>
     )}
