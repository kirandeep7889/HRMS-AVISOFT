...
import { DeleteManagerAddedReview, EditManagerAddedReview, ManagerAddedReviews } from '../../../../../services/operations/performanceAPI';
...
const PerformanceReviewList = () => {
  ...
  useEffect(() => {
    const fetchEmployeeReviews = async () => {
      const response = await dispatch(ManagerAddedReviews(ReviewerId, AccessToken));
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await dispatch(EditManagerAddedReview(performanceId, AccessToken, data));
  ...
  return (
    <div className="pb-9 bg-slate-100 rounded-md mb-5">
      ...
      <h1 className='text-center font-semibold text-blue-900 mt-10 text-2xl'>Performance Review List</h1>
      <div className="p-10">
        {reviews.length > 0 ? (
          <table className="table-auto w-full">
            <thead>
              <tr className="bg-gray-200">
                ...
                <th className="py-3 px-4 border-b-2 border-slate-300 text-sm leading-4 text-black font-semibold uppercase tracking-wider text-center">Action</th>
              </tr>
            </thead>
            <tbody>
              {reviews.map(review => (
                <tr key={review.performanceId} className={review.performanceId % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                  ...
                  <td className="py-3 px-4 border-b border-slate-200 text-center">{review.reviewDate}</td>
                  <td className="py-3 px-4 border-b border-slate-200 text-center">
                    <button onClick={() => handleEditClick(review)} className="mr-2 focus:outline-none">
                      <FontAwesomeIcon icon={faEdit} className="text-blue-500 hover:text-blue-700" />
                    </button>
                    <button 
                      onClick={() => setConfirmationModal({
                        text1: "Are you sure?",
                        text2: "You want to delete this selected performance review.",
                        btn1Text: "Delete Review",
                        btn2Text: "Cancel",
                        btn1Handler: async () => { await dispatch(DeleteManagerAddedReview(review?.performanceId, AccessToken)); refreshPage(); },
          ...
          <p className="text-center text-gray-600 mt-10">No performance reviews found.</p>
        )}
      </div>
      {showEditForm && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 w-96 ">
            <h3 className="text-xl font-semibold mb-4">Edit Performance Review</h3>
